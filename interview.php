		...
		<!-- Menu -->
			<nav id="menu">
				<ul class="links">
					...
					?>
					<?php
						if($_COOKIE['Ulogin']=='管理員' || $_COOKIE['Ulogin']=='口試' )
						{
					?>
							...
							<div class="table-wrapper" style="color:black;width:100%;">
								<form method="post" style="width:100%;" action="">
									<?php
										...
										{
											$SQL_DATA="SELECT * FROM `candidate` WHERE `Time`=\"".$_POST['time']."\" ORDER BY `candidate`.`Time` ASC, `candidate`.`round` ASC , `candidate`.`Number` ASC";
								...
								<form method="post" style="width:100%;" action="interviewsent.php">
									<table class="alt">
										...
										<tbody>
											<tr>
													<th style="text-align: center;width:10%;"><div style="overflow:auto;width:100%;height:120px;">准考證號<br>姓名<br>梯次(編號)</div></th>
										...
										<?php
											...
											while ($DATA_rowres = mysqli_fetch_array($DATA_result_status, MYSQLI_BOTH))
											{
												...
												<tr>
													<td rowspan="1" style="background-color:<?php echo $Color?>"><?php echo $DATA_rowres[0]."<br>".$DATA_rowres[3]."<br>".$DATA_rowres[5]."(".$DATA_rowres[7].")"?></td>
													<?php
													if($_COOKIE['Uname']=='nttuadmin'||$_COOKIE['Uname']=='nttucsieadmin2'||$_COOKIE['Uname']=='cwlee'){
													?>
